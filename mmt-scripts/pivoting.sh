    ...
    if [ $SRC != $PIVOT ] ; then
        for TGT in $LANGUAGES ; do
            if [ $TGT != $SRC ] ; then
                ...
                cat $RESULTS_PATH/hypotheses.$SRC-$TGT.$TGT/generate-test.txt | grep -P '^H-'  | cut -c 3- | sort -n -k 1 | awk -F "\t" '{print $NF}' | python $FAIRSEQ_PATH/scripts/spm_encode.py --model $FLORES_MODEL_PATH/sentencepiece.model.bpe > $PIVOT_DIR/data/$PIVOT/spm/$SRC.$PIVOT-$TGT.$PIVOT
                FLORES_TGT=$(echo $TGT | python $SCRIPTS_PATH/flores2m2m.py flores)
                fairseq-preprocess -s $PIVOT -t $TGT --testpref $PIVOT_DIR/data/$PIVOT/spm/$SRC.$PIVOT-$TGT --destdir $PIVOT_DIR/data/$PIVOT/bin/$SRC.$PIVOT-$TGT --srcdict $DICT --tgtdict $DICT --only-source
